...
- `RULE-14-3` - `ControllingExprInvariant.ql`: fixed a false negative where a loop's compound
  controlling expression that always evaluates to false (e.g. `1 == 0 && p1 > 12`) was incorrectly
  permitted by the exception for essentially-Boolean literal `0`/`0U` controlling expressions
  (e.g. `do { ... } while (0U == 1U)`).
