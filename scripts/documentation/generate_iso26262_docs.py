...
parser.add_argument("output_directory", help="the directory where the html files should be written")
...
env = Environment(loader=FileSystemLoader(Path(__file__).parent.joinpath("templates")), trim_blocks=True, lstrip_blocks=True)
...
def generate_html(markdown_file, html_output_file):
  ...
  try:
    ...
    data = json.dumps({'text': text, 'mode': 'gfm'}, ensure_ascii=False).encode('utf-8')
    response = requests.post("https://api.github.com/markdown", data=data, headers={'Accept': 'application/vnd.github.v3+json'})
    ...
    html_output = env.get_template("rendered_markdown.html.template").render(markdown_body=response.text, title=markdown_file.name)
    # Write the HTML page to the output file
    with open(html_output_file, "w", newline="\n") as f:
      f.write(html_output)
  except HTTPError as http_err:
    print("Error: could not process markdown file '" + str(markdown_file) + "'.")
...
for markdown_file in ["development_handbook.md", "docs/user_manual.md", "docs/iso_26262_tool_qualification.md", ".github/pull_request_template.md"]:
  generate_html(repo_root.joinpath(markdown_file), output_directory.joinpath(markdown_file).with_suffix(".html"))
