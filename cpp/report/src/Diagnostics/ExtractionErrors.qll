 ...
 *
 * NOTE: This is copied from the `ExtractionErrors.qll` library provided by `github/codeql` in
 * order to apply a few small bug fixes (identified by NOTE markers placed inline in the code).
 */
...
class ExtractionUnknownError extends ExtractionError, TUnknownError {
  ...
  override File getFile() {
    // NOTE: This is a change from the standard library vesion
    // If the error location is `file://0:0:0:0` then this is likely a fatal compilation error, and
    // we can more usefully report the file being compiled as the location of the error.
    if err.getLocation().hasLocationInfo("", 0, 0, 0, 0)
    then result = err.getCompilation().getAFileCompiled()
    else result = err.getFile()
  }
  ...
  override Location getLocation() {
    ...
    // we can more usefully report the file being compiled as the location of the error.
    if err.getLocation().hasLocationInfo("", 0, 0, 0, 0)
    then result = err.getCompilation().getAFileCompiled().getLocation()
    else result = err.getLocation()
  }
}
