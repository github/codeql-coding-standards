/**
 * THIS IS A MODIFIED COPY OF THE `semmle.code.cpp.valuenumbering.HashCons` LIBRARY.
 *
 * The library was copied from `github/codeql` at version `v1.26`.
 *
 * We make two modifications:
 *  1. Restrict the analysis to sub-expressions of field assignments in constructors. This reduces
 *     the peformance overhead of evaluating a second copy of `HashCons`.
 *  2. Consider the access of any parameter to be equivalent. This allows us to find to assignments
 *     to fields that are equivalent across different constructors.
 *
 * The changes are highlighted in the source code using ADDED and ADDED END markers.
 * ----
 *
...
/* ADDED */
predicate interestingExpr(Expr e) {
  exists(Constructor c, Field f, Expr assignedValue |
    not c.isCompilerGenerated() and
    assignedValue = f.getAnAssignedValue() and
    assignedValue.getEnclosingFunction() = c and
    e = assignedValue.getAChild*()
  )
}
...
private predicate mk_HasInit(HashCons hc, NewOrNewArrayExpr new) {
  /* ADDED */
  interestingExpr(new) and
  /* ADDED END */
  hc = hashCons(new.(NewExpr).getInitializer().getFullyConverted())
  or
...
private predicate mk_HasAlloc(HashCons hc, NewOrNewArrayExpr new) {
  ...
  interestingExpr(new) and
  /* ADDED END */
  hc = hashCons(new.(NewExpr).getAllocatorCall().getFullyConverted())
  or
...
HashCons hashCons(Expr e) {
  ...
  /* ADDED */
  exists(Parameter p |
    analyzableVariable(e) and
    p = e.(VariableAccess).getTarget() and
    result = HC_AnyParameter()
  )
  or
  /* ADDED END */
  ...
  result = HC_Nullptr()
  or
  not analyzableExpr(e, _) and
  /* ADDED */
  interestingExpr(e) and
  /* ADDED END */
  result = HC_Unanalyzable(e)
}
