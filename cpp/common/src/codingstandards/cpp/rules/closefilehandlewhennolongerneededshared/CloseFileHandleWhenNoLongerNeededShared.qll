 ...
 * file handles which are open but not closed before they go out of scope.
 */
...
abstract class CloseFileHandleWhenNoLongerNeededSharedSharedQuery extends Query { }

Query getQuery() { result instanceof CloseFileHandleWhenNoLongerNeededSharedSharedQuery }

query predicate problems(ControlFlowNode def, string message, Stmt ret, string retMsg) {
  not isExcluded(def, getQuery()) and
  message = "The file opened here may not be closed at $@." and
  ...
  (
    ...
    or
    opened(def) and
    not fopenCallMayBeClosed(def) and
