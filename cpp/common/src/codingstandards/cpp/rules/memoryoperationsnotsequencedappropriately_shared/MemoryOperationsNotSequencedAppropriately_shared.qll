...
abstract class MemoryOperationsNotSequencedAppropriately_sharedSharedQuery extends Query { }

Query getQuery() { result instanceof MemoryOperationsNotSequencedAppropriately_sharedSharedQuery }
...
query predicate problems(
  FullExpr fullExpr, string message, VariableEffect variableEffect1, string variableEffect1_string,
  Locatable placeHolder, string label, VariableAccess va1, string va1_string, VariableAccess va2,
  string va2_string
) {
  exists(ConstituentExprOrdering orderingConfig |
    not isExcluded(fullExpr, getQuery()) and
    isUnsequencedEffect(orderingConfig, fullExpr, variableEffect1, va1, va2, placeHolder, label) and
    message = "The expression contains unsequenced $@ to $@ and $@ to $@." and
    variableEffect1_string = "side effect" and
    va1_string = va1.getTarget().getName() and
    va2_string = va2.getTarget().getName()
  )
}
