...
class InitializationSubBasicBlock extends SubBasicBlockCutNode {
  InitializationSubBasicBlock() {
    any() or this = any(UninitializedSubobject uv).getADefinitionAccess()
  }
}

class RelevantObject extends ObjectIdentity {
  RelevantObject() {
    not this.getStorageDuration().isThread() and
    not this.getStorageDuration().isStatic()
  }

  Element getADeclaration() {
    result.(DeclStmt).getADeclaration() = this
    or
    result.(Expr) = this
  }
}

class RelevantSubObject extends SubObject {
  RelevantSubObject() { this.getRootIdentity() instanceof RelevantObject }

  Expr getAnAssignedValue() {
    exists(Assignment a | a.getLValue() = this.getAnAccess() | result = a.getRValue())
  }

  Expr getInitializerExpr() {
    this.representsRootObject() and
    result = this.getRootIdentity().(LocalVariable).getInitializer().getExpr()
    or
    this.representsRootObject() and
    result = this.getRootIdentity().(AllocatedObjectIdentity) and
    exists(this.getRootIdentity().(NewExpr).getAChild())
  }

  string getName() {
    this.representsRootObject() and
    result = this.getRootIdentity().toString()
    or
    result = toString()
  }

  predicate isFromUninstantiatedTemplate() {
    this.getRootIdentity().(Variable).isFromUninstantiatedTemplate(_)
  }

  Element getParentScope() {
    result = this.getRootIdentity().(LocalVariable).getParentScope()
    or
    result = this.getRootIdentity().(Expr).getParentScope()
  }
}
...
private newtype TInitializationContext =
  ...
  NoContext(UninitializedSubobject uv) { count(uv.getACorrelatedConditionVariable()) = 0 } or
  /**
   * A context where the given `LocalScopeVariable` is identified as a correlated variable with the
   * given state value.
   */
  CorrelatedVariable(UninitializedSubobject uv, LocalScopeVariable correlatedVariable, boolean state) {
 ...
 *
 * If no suitable context is found for an `UninitializedSubobject`, `NoContext` is used.
 *
...
class InitializationContext extends TInitializationContext {
  /**
   * Gets the `UninitializedSubobject` for this context.
   */
  UninitializedSubobject getUninitializedSubobject() {
  ...
  string toString() {
    ...
    then
      result =
        "Uninitialized variable " + getUninitializedSubobject().getName() + " where location " +
          getCorrelatedVariable().getName() + " is " +
          any(boolean b | this = CorrelatedVariable(_, _, b))
    else result = "Uninitialized variable " + getUninitializedSubobject().getName()
  }
}
 ...
 */
class UninitializedSubobject extends RelevantSubObject {
  UninitializedSubobject() {
    // Not initialized at declaration
    not exists(getInitializerExpr()) and
    ...
    not isFromUninstantiatedTemplate() and
    ...
    not this.getRootIdentity() = any(RangeBasedForStmt f).getAChild().(DeclStmt).getADeclaration()
  }
  ...
  Expr getAUse() {
    ...
    /* COMMENTED OUT THE FOLLOWING because we have tests that assume this condition doesnt exist! */
    // Not a pointless read
  ...
  Expr getAnUnitializedUse() {
    exists(SubBasicBlock useSbb |
      ...
      forex(InitializationContext ct | ct.getUninitializedSubobject() = this |
  ...
  private SubBasicBlock getAnUninitializedSubBasicBlock(InitializationContext ic) {
    ic.getUninitializedSubobject() = this and
    (
      // Base case - this SBB is the one that declares the variable
      exists(RelevantObject root |
        root = this.getRootIdentity() and
        result.getANode() = root.getADeclaration()
      )
      or
...
query predicate problems(Expr va, string message, UninitializedSubobject uv, string name) {
  not isExcluded(va, getQuery()) and
  name = uv.getName() and
  va = uv.getAnUnitializedUse() and
  message = "Object $@ is read here and may not be initialized on all paths."
}
