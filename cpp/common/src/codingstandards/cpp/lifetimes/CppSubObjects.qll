/**
 * A library for tracking "subobjects" of objects in C++.
 *
...
newtype TSubObject =
  ...
  TObjectMember(SubObject struct, MemberVariable m) {
    m = struct.getType().(Class).getAMemberVariable() and
    // References point to subobjects of the reference type, so fields of reference type are not
    // subobjects of the containing object:
    not m.getUnderlyingType() instanceof ReferenceType
  } or
...
class SubObject extends TSubObject {
  ...
  predicate representsRootObject() { this = TObjectRoot(_) }
}
