 ...
 *    is given below.
 *
 *    - Basically, we generate `T(const T &x) : m(x.m) ... {}` for copy ctors, the same but `T &&x`
 *      for move constructors, and `operator=(const T &x) { m = x.m; ... }` for copy assignment, and
 *      the same but `T &&x` for move assignment. Overload resolution must succeed in this generated
 *      code for each constructor `M(x.m)` and assignment `m = x.m`.
 *
 *    - Array types and handled element-wise, e.g. `arr[0] = x.arr[0]; ...`. If the type of `x::M`
 *      is an rvalue reference, then the generated code is `x = static_cast<M&&>(x)`.
 *
 *    - The first note is that `x` in `x.m` as shown above always has reference type (`T&&`) or
 *      reference to const type (`const T&`) which is adjusted to `T` or `const T` [expr]/5. Both
 *      are lvalues [expr.prim.id.unqual]/1.
 *
 *    - Next we consider `.m`. If the raw type of `T::m` is "reference to M", then the `x.m` is an
 *      lvalue of type M [expr.ref]/4. (Note that `M` may by `const M'` and the const is preserved.)
 *      Otherwise, `x.m` has type `const M` in a copy constructor and `M` in a move constructor.
 *      [expr.ref]/4.2
 *
